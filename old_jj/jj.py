...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Page label and desc
        tk.Label(self, text='_______________________________________________', font=("Helvetica", "16")).place(relx=0.5, y=64, anchor=N)
        ...
        def next():
            ...
            if c1.get() or c2.get() or c3.get():
                ...
                # Clear error message
                err_label = Message(self, text = '\t\t\t\t\t\t\t\t\t', font=SMALL_FONT, width=420)
                err_label.place(relx=0.5, y=cb_pos+130, anchor=CENTER)

            # Display error if missing mandatory arg
            else:
                err_label = Message(self, text = 'Error. You must select at least one option.', font=SMALL_FONT, width=420)
...
class adv_page(tk.Frame):
    def __init__(self, parent, controller):
        ...
        procs_lab.place(relx=0.19, y=250, anchor=CENTER)
        crawl_lab = Label(self, text="Max crawl depth: ", font=SMALL_FONT)
        crawl_lab.place(relx=0.23, y=280, anchor=CENTER)
...
class job_page(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Page label
        label = tk.Label(self, text='Enter jobs to search for', font=LARGE_FONT)
        label.place(relx=0.5, y=title_y, anchor=CENTER)
...
class zip_page(tk.Frame):
    def __init__(self, parent, controller):
            ...
            if len(home_resp) > 0:
                try:
                    ...
                    if not home_resp in zip_dict:
                        ...
                        # Clear error message
                        Label(self, text="\t\t\t\t\t\t\t\t\t", font=SMALL_FONT).place(relx=0.5, y=err_y, anchor=CENTER)
        ...
        L1.place(relx=0.27, y=entry_y, anchor=CENTER)
...
class dist_page(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Page label
        label = tk.Label(self, text='Enter the maximum distance (in miles) to search', font=LARGE_FONT)
        label.place(relx=0.5, y=title_y, anchor=CENTER)
        label2 = tk.Label(self, text='Leave blank to search all of NYS.', font=SMALL_FONT)
        label2.place(relx=0.5, y=desc_y, anchor=CENTER)
...
class results_page(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Display results
        def update_results():
            a = tk.Label(self, text='Matches found at:', font=LARGE_FONT)
            a.place(relx=0.5, y=20, anchor=CENTER)

            # Use try to ignore errors?
            try:

                # Scrollable text field
                editArea = tkst.ScrolledText(self, width=60, height=16, font=SMALL_FONT, wrap="none")
                editArea.place(x=15, y=40)
                ...
                tk.Label(self, text=brow_text, font=SMALL_FONT).place(relx=0.5, y=380, anchor=CENTER)

                # Next button
                if len(portal_error_list) > 0:
                    nextButton = Button(self, text="More", command=lambda : controller.show_frame(portal_error_page), font=SMALL_FONT)
                    nextButton.place(relx=0.8, y=420, anchor=CENTER)
        ...
        Button(self, text="Open in browser", command=o_i_b, font=SMALL_FONT).place(relx=0.5, y=420, anchor=CENTER)
...
class portal_error_page(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Display results
        def update_results():
            a = tk.Label(self, text='Portal errors at:', font=LARGE_FONT)
            a.place(relx=0.5, y=20, anchor=CENTER)
            ...
            try:
                ...
                # Browser label
                brow_text = 'Unable to search these ' + str(len(portal_error_list)) + ' portal URLs.\nOpen them in your browser?'
                tk.Label(self, text=brow_text, font=SMALL_FONT).place(relx=0.5, y=380, anchor=CENTER)
