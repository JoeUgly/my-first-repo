# Description: Search NYS civil service and school webpages for keywords using a GUI.

# To do:
# help button and/or context menu
# pages for portal errors, domain exceedances, stats
# make pretty cross platform
# browser may not open all results
# clear job entries button
# empty zip search +
# select which URL results to open. need checkboxes scrolling with results
# identify and merge changes with cli
# back to grid for cross plat formatting
# test on Mint
...
blacklist = ['herkimercounty.org/content/departments/view/9:field=services;/content/departmentservices/view/190', 'herkimercounty.org/content/departments/view/9:field=services;/content/departmentservices/view/35', 'http://cs.monroecounty.gov/mccs/lists', 'https://cs.monroecounty.gov/mccs/lists', 'https://countyherkimer.digitaltowpath.org:10069/content/departments/view/9:field=services;/content/departmentservices/view/190', 'https://countyherkimer.digitaltowpath.org:10069/content/departments/view/9:field=services;/content/departmentservices/view/35', 'https://www.monroecounty.gov/mccs/lists', 'https://www.tompkinscivilservice.org/civilservice/jobs', 'http://www.co.essex.ny.us/personnel', 'http://www.monroecounty.gov/hr/lists', 'http://www.niagaracounty.com/employment/eligible-lists', 'http://www.ogdensburg.org/index.aspx?nid=345', 'http://www.tompkinscivilservice.org/civilservice/jobs', 'www.penfield.org/multirss.php', 'https://jobs.albanyny.gov/default/jobs', 'https://mycivilservice.rocklandgov.com/default/jobs', 'http://www.co.ontario.ny.us/94/Human-Resources', 'http://www.cc.cnyric.org/districtpage.cfm?pageid=112']

# High confidence
jobwords_certainty = ['employment', 'job opening', 'job posting', 'job announcement', 'job opportunities', 'jobs available', 'available positions', 'available employment', 'career opportunities', 'employment opportunities', 'current vacancies', 'current job', 'current employment', 'current opening', 'current posting', 'current opportunities', 'careers at', 'jobs at', 'work at', 'employment at', 'find your career']

# Low confidence
jobwords_civ = ['continuous recruitment', 'open competitive', 'open-competitive', 'open to', 'opportunities', 'browse jobs', 'search jobs', 'job', 'job seeker', 'job title', 'civil service', 'civil service opportunities', 'civil service exam', 'civil service test', 'civil service exam announcements', 'exam', 'examination', 'exam announcement', 'examination announcement', 'test', 'position', 'career', 'human resource', 'personnel', 'vacancies', 'current exam', 'current civil service', 'posting', 'opening', 'vacancy']

jobwords_s = ['join', 'opportunities', 'browse jobs', 'search jobs', 'job seeker', 'job', 'job title', 'position', 'career', 'human resource', 'personnel', 'vacancies', 'posting', 'opening', 'faculty', 'staff', 'adjunct', 'recruitment', 'academic', 'support', 'instructional', 'administrative', 'professional', 'coaching', 'vacancy']

bunkwords = ['pnwboces.org', 'recruitfront.com', 'schoolapp.wnyric.org', 'professional development', 'career development', 'javascript:', '.pdf', '.jpg', '.ico', 'icon', '.doc', 'mailto:', 'tel:', 'description', 'specs', 'specification', 'guide', 'faq', 'images', 'exam scores', 'resume-sample', 'resume sample', 'directory', 'pupil personnel']
...
def scraper(keyword_list, all_urls_q, max_crawl_depth, keywordurl_man_list, checkedurls_man_list, errorurls_man_dict, skipped_pages, prog_count, total_count, all_links_arg, verbose_arg, jobwords):
    ...
    while True:
        ...
        # Begin fetching
            ...
            try:
                ...
                if domain_delim > -1: domain = workingurl[:domain_delim]
                else: domain = workingurl
                ...
                # Catch errors
                    ...
                    if 'timed out' in str(errex):
                        ...
                        # Use domain as portal error fallback
                ...
                # Attempt latin-1 charset encoding
                    ...
                    try:
                        ...
                        with lock:
                            errorurls_man_dict[workingurl] = ['error 2: ', str(errex)[:999], current_crawl_level]
                        continue
                ...
                if current_crawl_level != -1:
                    ...
                    if any(zzzz in dec_html for zzzz in keyword_list):
                        ...
                        jbw_count = 0
                        for i in jobwords:
                            if i in dec_html: jbw_count += 1
                        for i in jobwords_certainty:
                            if i in dec_html: jbw_count += 2
                ...
                if current_crawl_level < max_crawl_depth:
                    ...
                    alltags = re.findall('href=.*?</a>', up_dec_html, flags=re.DOTALL)
                    alltags_set = set(alltags)
                    
                    # Set jbws confidence based on tags
                    high_confidence = False
                    for tag in alltags_set:
                        if any(www in tag for www in jobwords_certainty):
...
##
                        ...
                        if not all_links_arg:
                            ...
                            # Proceed if the tag contains a high confidence jobword
                            if high_confidence:
                            ...
                            # Set jbws type based on original queue put
                                ...
                                # Proceed if the tag contains a low confidence jobword                                
                                if not any(yyy in lower_tag for yyy in jobwords):
                                    continue                                
                        ...
                        # Use lowercase for dup checking only
                        abspath_lower = abspath.lower()
                        
                        # Remove scheme and www to prevent dups
                        dup_checker = abspath_lower
                        if dup_checker.count('://') > 0:
                            dup_checker = dup_checker.split('://')[1]
                        ...
                        if abspath_lower in blacklist:
                            if verbose_arg: print(os.getpid(), 'Blacklist invoked at:', abspath_lower)
                            continue
...
# Multiprocessing
def starter(keyword_list, all_links_arg, write_arg, verbose_arg, num_procs, max_crawl_depth, home_resp, max_dist, civ_arg, school_arg, uni_arg, prog_count, total_count, finalkeywordurl_list, glob_act_chil, portal_error_list):
    ...
    # Set default values
    try: all_links_arg
    except: all_links_arg = False
    try: write_arg
    except: write_arg= False
    try: verbose_arg
    except: verbose_arg = False
    try: num_procs
    except: num_procs = 32
    try: max_crawl_depth
    ...
    '''
    if checks[0]: civ_arg = True
    else: civ_arg = False
    if checks[1]: school_arg = True
    else: school_arg = False        
    if checks[2]: uni_arg = True
    else: uni_arg = False
    '''
    ...
    # Associate input to a ZIP's coords if not blank
    if home_resp:
        hc = zip_dict[home_resp]
    ...
    if school_arg:
        for the_list in school_list:
            ...
            if home_resp:
                ...
                # Append jbws type if mixing search parameters
    ...
    total_count.value += qlength



    ## why use man list at all?
    # Create manager lists
    manager = Manager()
    keywordurl_man_list = manager.list()
    checkedurls_man_list = manager.list()
    errorurls_man_dict = manager.dict()
    #finalkeywordurl_list = manager.list()

    # Create child processes
    for ii in range(num_procs):
        worker = Process(target=scraper, args=(keyword_list, all_urls_q, max_crawl_depth, keywordurl_man_list, checkedurls_man_list, errorurls_man_dict, skipped_pages, prog_count, total_count, all_links_arg, verbose_arg, jobwords))
        worker.start()

    # Wait until all tasks are done
    current_prog_c = None
    while len(active_children()) > 1:
        glob_act_chil.value = len(active_children())
    ...
    for entry in keywordurl_man_list:
        ...
        # Remove scheme, www., and '] to prevent dups
    ...
    for each_portal in domain_bare_dict.keys():
        each_domain_list = []
        domain_count = 0
        ...
        if len(domain_bare_dict[each_portal]) > domain_limit_num:
            each_domain_list = domain_bare_dict[each_portal]
        ...
        # If the number of URLs is below the threshold then append all results to final good list
            ...
            for i in domain_bare_dict[each_portal]:
                finalkeywordurl_list.append(i)
    ...
    # Build portal URL errors list based on crawl level and if in a portal list
    #portal_error_list = []
    if len(errorurls_man_dict.items()) > 0:
        for v,k in errorurls_man_dict.items():
            
            ## Occasional index out of range error maybe fixed by adding current crawl level to all errors
            if k[2] < 1:
...
['http://southbronxclassical.org/careers/opportunities', 0, (40.85048545, -73.8404035580209)],
...
['http://www.dfsd.org/domain/239 :: http://www.dfsd.org/Page/1142 :: http://www.dfsd.org/domain/309', 0, (41.0145418, -73.872635)],
...
['http://www.greece.k12.ny.us', 0, (43.1854754, -77.6106861508176)],
...
['http://www.southlewis.org/Page/1008', 0, (43.627378, -75.41153)],
...
['https://urbanassembly.org/career :: https://urbanassembly.org/mission', 0, (40.7308619, -73.9871558)],
...
['https://www.hancock.stier.org/domain/49', 0, (41.9539754, -75.2804502)],
...
['http://www.rochester.edu/working/hr/jobs', 0, (43.157285, -77.615214)],
...
['https://employment.potsdam.edu', 0, (44.6752342, -74.9860333)],
...
['https://jobs.plattsburgh.edu/postings/searchu', 0, (44.69282, -73.45562)],
...
['https://www.paycomonline.net/v4/ats/web.php/jobs?clientkey=742F9A03129057C86AF9C8D229C5EE14&jpt=', 0, (40.7308619, -73.9871558)],
...
class MainWindow(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        # Each page
        for F in (StartPage, job_page, zip_page, dist_page, adv_page, search_page, results_page, portal_error_page):
...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Draw a line
        w = Canvas(self, width=400, height=40)
        w.grid(row=3, column=1, sticky="nsew")
        w.create_line(10, 10, 590, 10)

        # Page label and desc
        tk.Label(self, text='_______________________________________________\n', font=("Helvetica", "16")).grid(row=4, column=1, sticky="new")        
        title_label = tk.Label(self, text='Joe\'s Jorbs')
        title_label.config(font=("Helvetica", "24", "bold"))
        title_label.grid(row=1, column=1)
        desc_label = tk.Label(self, text='Find jobs in New York State', font=("Helvetica", "16", "bold"))
        desc_label.grid(row=2, column=1)

        # Message
        cb_pos = 150
        messageVar = Message(self, text = 'Select at least one to search', font=(LARGE_FONT), width=380) 
        messageVar.grid(row=4, column=1, sticky="nsew")
        ...
        # Create and place parameter checkbuttons
        tk.Checkbutton(self,text='Civil Service', font=LARGE_FONT, variable=c1).grid(row=5, column=1, sticky="nsew")
        tk.Checkbutton(self,text='Schools        ', font=LARGE_FONT, variable=c2).grid(row=6, column=1, sticky="nsew")
        tk.Checkbutton(self,text='Universities ', font=LARGE_FONT, variable=c3).grid(row=7, column=1, sticky="nsew")

        tk.Label(self, text='\n', font=("Helvetica", "16")).grid(row=8, column=1, sticky="nsew")
        ...
        # Must set intial values for search parameters
        global civ_arg
        civ_arg = False
        global school_arg        
        school_arg = False
        ...
        def next():
            ...
            if c1.get() or c2.get() or c3.get():
                ...
                # Clear error message
                err_label = Message(self, text = '\t\t\t\t\t', font=SMALL_FONT, width=200) 
                err_label.grid(row=8, column=1, sticky="nsew")

            # Display error if missing mandatory arg
            else:
                err_label = Message(self, text = 'Error. You must select at least one option.', font=SMALL_FONT, width=200) 
...
# Set initial values for adv options
all_links_arg = False
write_arg = False
verbose_arg = False
num_procs = 32
...
class adv_page(tk.Frame):
    def __init__(self, parent, controller):
        ...
        procs_lab.grid(row=7, column=0, sticky='e')
        crawl_lab = Label(self, text="Max crawl depth: ", font=SMALL_FONT)
        crawl_lab.grid(row=8, column=0, sticky='e')
        ...
        def back():
            ...
            global all_links_arg
            global write_arg
            global verbose_arg

            # Set default values if blank
            num_procs = procs_entry.get()
            if len(num_procs) < 1: num_procs = 32
            num_procs = int(num_procs)
            
            max_crawl_depth = crawl_entry.get()
            if len(max_crawl_depth) < 1: max_crawl_depth = 2
            max_crawl_depth = int(max_crawl_depth)

            ## remove verbose?
            all_links_arg = c4.get()
            write_arg = c5.get()
            verbose_arg = c6.get()
            controller.show_frame(StartPage)

        # Create and place checkbuttons
        tk.Checkbutton(self,text='All links      ', font=LARGE_FONT, variable=c4).grid(row=3, column=1)
        tk.Checkbutton(self,text='Write to file', font=LARGE_FONT, variable=c5).grid(row=4, column=1)
        tk.Checkbutton(self,text='Verbose     ', font=LARGE_FONT, variable=c6).grid(row=5, column=1)

        tk.Label(self, text='', font=LARGE_FONT).grid(row=6, column=1)    
        
        # Number of processes
        global procs_entry
        procs_entry = Entry(self, bd=4, font=SMALL_FONT)
        procs_entry.insert(END, '32')
        procs_entry.grid(row=7, column=1)
        Label(self, text=" Default: 32", font=SMALL_FONT).grid(row=7, column=2, sticky='w')

        # Max crawl depth
        global crawl_entry
        crawl_entry = Entry(self, bd=4, font=SMALL_FONT)
        crawl_entry.insert(END, '2')
        crawl_entry.grid(row=8, column=1)
        Label(self, text=" Default: 2", font=SMALL_FONT).grid(row=8, column=2, sticky='w')

        # Back button
        backButton = Button(self, text="Accept and Back", command=back, font=SMALL_FONT)
        backButton.place(x=70,y=330)        
...
class job_page(tk.Frame):
    def __init__(self, parent, controller):
        ...
        L1.grid(row=2, column=0, sticky="nse")
        L_ex = Label(self, text=" Example: librarian", font=SMALL_FONT)
        L_ex.grid(row=2, column=2, sticky="nsw")
        global job_entry
        job_entry = Entry(self, bd =4, font=SMALL_FONT)
        job_entry.grid(row=2, column=1, sticky="nsew")
...
class zip_page(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label2.grid(row=1, column=0, columnspan=3, sticky="ns")        
        ...
        def next():            
            ...
            if len(home_resp) > 0:
                try:
                    home_resp = int(home_resp)

                    # Catch ZIP not found
                    if not home_resp in zip_dict:
                        ...
                        # Clear error message
                        Label(self, text="\t\t\t\t\t", font=SMALL_FONT).grid(row=3, column=0, columnspan=3)
                        
                        # Set ZIP and go to next page
                        z_label = Label(self, text="Using ZIP code: " + str(home_resp), font=SMALL_FONT)
                        z_label.grid(row=3, column=1, sticky="nsew")
            ...
            ## Exit if ZIP is blank
                ...
                ## move this to global function to allow empty ZIP? -
                # Set man objects to be passed to scraper and progress page
                manager = Manager()
                global prog_count
                prog_count = Value('i', 0)
                global total_count
                total_count = Value('i', 0)
                global finalkeywordurl_list
                finalkeywordurl_list = manager.list()
                global portal_error_list
                portal_error_list = manager.list()
                
                controller.show_frame(search_page)

                # Start the program in another concurrent proc otherwise the GUI locks up
                worker = Process(target=starter, args=(keyword_list, all_links_arg, write_arg, verbose_arg, num_procs, max_crawl_depth, home_resp, max_dist, civ_arg, school_arg, uni_arg, prog_count, total_count, finalkeywordurl_list, glob_act_chil, portal_error_list))
                worker.start()                
        ...
        zip_entry.grid(row=2, column=1, sticky="nsew")
...
class dist_page(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Create a man value to act as a global proxy for the active children value
        global glob_act_chil
        glob_act_chil = Value('i', 2)
        ...
        L_ex.grid(row=2, column=2, sticky='w')
        global dist_entry        
...
class search_page(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Page labels
        base_y = 20
        text2 = 'Max crawl depth = ' + str(max_crawl_depth)
        tk.Label(self, text=text2, font=SMALL_FONT).place(relx=0.1, y=base_y+30)
        tk.Label(self, text='Waiting for all processes to finish.', font=LARGE_FONT).place(relx=0.5, y=base_y+270, anchor=CENTER)           
        
        # Real time display
        def update_labels():
            
            # Variable postion page labels
            tk.Label(self, text='\t\t\t\t', font=SMALL_FONT).place(relx=0.5, y=base_y, anchor=CENTER)
            text1 = 'Number of processes running = ' + str(glob_act_chil.value)
            tk.Label(self, text=text1, font=SMALL_FONT).place(relx=0.1, y=base_y)
            text3 = 'Now searching for keyword(s): ' + str(keyword_list)[1:-1]
            tk.Label(self, text=text3, font=SMALL_FONT).place(relx=0.1, y=base_y+60)

            ## cover up prev checkmark 
            # Add 30 to the y pos for every search parameter used
            y_count = base_y + 120
            tk.Label(self, text='Searching in:', font=SMALL_FONT).place(relx=0.1, y=base_y+90)
            if civ_arg: 
                tk.Label(self, text='Civil Service\t\t\t', font=SMALL_FONT).place(relx=0.1, y=y_count)
                y_count += 30
            if school_arg: 
                tk.Label(self, text='School districts and charter schools', font=SMALL_FONT).place(relx=0.1, y=y_count)
                y_count += 30                
            if uni_arg: 
                tk.Label(self, text='Universities and colleges\t', font=SMALL_FONT).place(relx=0.1, y=y_count)            
                            
            # Set progress label ignoring errors
            try:
                p_text = StringVar()
                p_text.set(prog_count.value)
                t_text = StringVar()
                t_text.set(total_count.value)
                b_text = 'Progress = ' + str(p_text.get()) + ' of ' + str(t_text.get())
            except:
                b_text = 0
                
            # Place progress label
            label = tk.Label(self, text=b_text, font=SMALL_FONT)
            label.place(relx=0.5, y=320, anchor=CENTER)
            
            # Go to next page when the search concludes
            if glob_act_chil.value < 2:
                controller.show_frame(results_page)
            
            # Update every 3 secs
            if glob_act_chil.value > 1:
                label.after(3000, update_labels)
...
class results_page(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Left column filler
        tk.Label(self, text='  ', font=LARGE_FONT).grid(row=0, column=0)
        ...
        def update_results():
            ...
            try:
                ...
                editArea = tkst.ScrolledText(self, width=56, height=14, font=SMALL_FONT, wrap="none")
                editArea.grid(row=1, column=1)
                
                # Put results into field
                for i in finalkeywordurl_list:
                    i = i[0] + '\n\n'
                    editArea.insert(tk.INSERT, i)
                
                # Browser label
                brow_text = 'Open all ' + str(len(finalkeywordurl_list)) + ' results in your browser?'
                tk.Label(self, text=brow_text, font=SMALL_FONT).grid(row=2, column=1)

                # Next button
                if len(portal_error_list) > 0:
                    nextButton = Button(self, text="More", command=lambda : controller.show_frame(portal_error_page), font=SMALL_FONT)
                    nextButton.grid(row=3, column=1, sticky='e')  
                
            except: pass
            
            # Stop the loop when the search concludes
            if glob_act_chil.value > 1:
                a.after(3000, update_results)
        ...
        # Open in browser
        def o_i_b():
            webbrowser.open(finalkeywordurl_list[0][0])
            time.sleep(4)
            for i in finalkeywordurl_list[1:]:
                webbrowser.open_new_tab(i[0])
                time.sleep(.2)
                
        # Open in browser button
        Button(self, text="Open in browser", command=o_i_b, font=SMALL_FONT).grid(row=3, column=1)
        

# More results page
class portal_error_page(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        ...
        # Display results
        def update_results():
            a = tk.Label(self, text='\nUnable to search these portal URLs:\n', font=LARGE_FONT)
            a.grid(row=0, column=1)
            ...
            try:    
                ...
                # Browser label
                brow_text = 'Open all ' + str(len(portal_error_list)) + ' portal URLs in your browser?'
                tk.Label(self, text=brow_text, font=SMALL_FONT).grid(row=2, column=1)
        ...
        # Open in browser
        def o_i_b():
            webbrowser.open(portal_error_list)
            time.sleep(4)
            for i in portal_error_list[1:]:
        ...
        # Back button
        nextButton = Button(self, text="Back", command=lambda : controller.show_frame(results_page), font=SMALL_FONT)
        nextButton.grid(row=3, column=1, sticky='w')        


# Run the GUI
if __name__ == '__main__':
    ## global vars down here?
    keyword_list = []
...
'''
        ...
        # Open in browser
        def o_i_b():
            webbrowser.open(portal_error_list[0])
            time.sleep(4)
            for i in portal_error_list[1:]:
                webbrowser.open_new_tab(i)
                time.sleep(.2)
