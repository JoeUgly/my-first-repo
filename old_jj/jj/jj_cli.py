...
# To do:
# xhr and fetch data collection has false positives?
# fix disgusto barfo
# high resource usage and long wait near end of program
# test
...
# Later versions:
# sel timeout, retry, error catch, and write rejects to file
# remove jj_error 2
# organize errorlog by url first
# dynamic proc and sel limits. sel limit = num_procs / 2? If ram usage greater than 60% and sel_count > 0?
...
# document which orgs use a centralized service and exlude them from jj search
# include more collapsible elements into selenium conf_f
...
# Things to do before freeze
# clear screen on progress refresh
# don't print jbw tally, dynamic content urls,



import datetime, gzip, os, queue, re, socket, time, traceback, urllib.parse, urllib.request, webbrowser, ssl
...
def dup_checker_f(dup_checker):
    ...
    # Catch no scheme error
        ...
        with lock:
            if verbose_arg: print('jj_error 2: No scheme:', dup_checker)
...
# Define proceed and optionally add to queue
def proceed_f(abspath, working_list, checkedurls_man_list, verbose_arg, skipped_pages, blacklist, current_crawl_level, all_urls_q, total_count, add_bool):
    ...
    if dup_checker in checkedurls_man_list:
        if verbose_arg: print('Skipping:', dup_checker)
        with lock:
    ...
    if dup_checker in blacklist:
        if verbose_arg: print('Blacklist invoked:', dup_checker)
        with lock:
            skipped_pages.value += 1
        return False

    # Add abspath to queue if add_bool is True
    if add_bool:
        ...
        if verbose_arg: print('Putting list into queue:', new_working_list)
    ...
    # Add dup_checker to checked pages list
    with lock:
...
def html_requester(workingurl, current_crawl_level, errorurls_man_dict, all_urls_q, total_count, domain, verbose_arg, working_list):
    ...
    # Catch and log HTTP request errors
        ...
        if 'timed out' in str(errex):
            if verbose_arg: print('jj_error 3: Request timeout:', workingurl)

            # Append to error log
            add_errorurls_f(workingurl, 'jj_error 3', str(errex), current_crawl_level, errorurls_man_dict)
        ...
        # Don't retry on 404 or 403 error
            ...
            if verbose_arg: print('jj_error 4: HTTP 404/403 request:', workingurl)
            add_errorurls_f(workingurl, 'jj_error 4', str(errex), current_crawl_level, errorurls_man_dict)
        ...
        # Retry on other error
            ...
            if verbose_arg: print('jj_error 5: Other request', workingurl)
            add_errorurls_f(workingurl, 'jj_error 5', str(errex), current_crawl_level, errorurls_man_dict)
            return True


# Define confirmation function using Selenium
def confirmation_f(sel_q, keyword_list, verbose_arg, keywordurl_man_list, sel_count):

    # Limit instances of Selenium
    if sel_count.value >= 8:
        return False
    ...
    if verbose_arg: print(os.getpid(), '==== Start Selenium function ====', sel_count.value)

    while True:

        # sel_list contents: [URL, jobword confidence, and portal URL]
        try:
            with lock:
                sel_list = sel_q.get(False)
        ...
        sel_url = sel_list[0]
        if verbose_arg: print(os.getpid(), 'sel_url:', sel_url)
        ...
        # Catch Sel request errors
        except Exception as errex:
            print('Selenium request error:', sel_url, errex)
            add_errorurls_f(workingurl, 'jj_error 7', '(a): ' + str(errex), current_crawl_level, errorurls_man_dict)
            with lock:
                sel_rejected_man_list.append(sel_url)
            driver.close()
            continue
        ...
        if not str(sel_text).strip():
            ...
            if not str(sel_text).strip():
                print('CSS and Xpath selectors have failed:', sel_url)
                add_errorurls_f(workingurl, 'jj_error 7', '(b): ' + str(errex), current_crawl_level, errorurls_man_dict)
                with lock:
                    sel_rejected_man_list.append(sel_url)
                driver.close()
                continue
        ...
        if len(coll_elems) > 0:
            ...
            # Expand all collapsible elements and wait for content to load
            for i in coll_elems:
                i.click()
                time.sleep(.5)

            ## Must append after each click because only one elem is visible at a time (will have duplicate data)
            # Append new content from collapsible element to the visble body text
        ...
        if any(sss in str(sel_text).lower() for sss in keyword_list):
            if verbose_arg: print('~~~~~~ Keyword confirmed:', sel_url)

            # Append URL, jobword confidence, and portal URL to result list
            with lock:
                keywordurl_man_list.append([sel_url, sel_list[1], sel_list[2]])
...
# Define add item to errorurls_man_dict function
# Use this to allow multiple errors (values) to each URL (key)
def add_errorurls_f(workingurl, err_code, err_desc, current_crawl_level, errorurls_man_dict):
...
def scraper(keyword_list, all_urls_q, max_crawl_depth, keywordurl_man_list, checkedurls_man_list, errorurls_man_dict, skipped_pages, prog_count, total_count, all_links_arg, verbose_arg, sel_q, sel_count, jbw_tally_man_l, dyn_urls_man_l):
    if verbose_arg: print(os.getpid(), '====== Start scraper function ======')
    while True:
        ...
        # If queue is empty wait and try again
            ...
            # Start confirmation function if queue is empty again
            except queue.Empty:
                sel_break = confirmation_f(sel_q, keyword_list, verbose_arg, keywordurl_man_list, sel_count)

                # Exit if both queues are empty or sel limit is met
                if sel_break:
                    if verbose_arg: print(os.getpid(), 'Both queues empty. Closing process')
                    break
                else:
                    if verbose_arg: print(sel_count.value, 'Selenium limit met. Closing process')
                    break
        ...
        # Begin fetching
            ...
            try:
                ...
                # Skip checked pages
                #add_bool = False
                #proceed_pass = proceed_f(workingurl, working_list, checkedurls_man_list, verbose_arg, skipped_pages, blacklist, current_crawl_level, all_urls_q, total_count, add_bool)
                ...
                for loop_count in range(3):
                    ...
                    if html == True:
                        if verbose_arg: print('Retry request loop:', workingurl)
                        continue

                    # html_requester returns False to indicate don't retry
                    elif html == False:
                        if verbose_arg: print('Break request loop:', workingurl)
                        break
                    ...
                    try:
                        ...
                        loop_success = True
                        if verbose_arg: print('Request and decode success:', workingurl)
                        break
                    ...
                    # Attempt explicit gzip decompression on decode error
                        ...
                        if "codec can't decode byte" in str(errex):
                            if verbose_arg: print(errex, 'Failed to decode. Retrying with gzip:', workingurl)

                            ## try saving two versions of html response to use here?
                            # Must make another request
                            html = html_requester(workingurl, current_crawl_level, errorurls_man_dict, all_urls_q, total_count, domain, verbose_arg, working_list)
                            if verbose_arg: print('Request success(2):', workingurl)

                            # Retry loop on request error
                            if html == True:
                                if verbose_arg: print('Retry request loop(2):', workingurl)
                                continue

                            # Exit loop on fatal error
                            elif html == False:
                                if verbose_arg: print('Break request loop(2):', workingurl)
                                break
                            ...
                            try:
                                ...
                                # Declare successful and exit loop
                                loop_success = True
                                if verbose_arg: print('Decode with gzip success:', workingurl)
                                break

                            # Catch other errors
                            except Exception as errex:
                                if verbose_arg: print('jj_error 6: gzip failure:', workingurl, errex)
                                add_errorurls_f(workingurl, 'jj_error 6', '(a): ' + str(errex), current_crawl_level, errorurls_man_dict)
                                continue

                        # Retry on other decode error
                        else:
                            charset_encoding = 'FAILED'
                            if verbose_arg: print('jj_error 6: ', charset_encoding, 'decode:', workingurl, errex)
                            add_errorurls_f(workingurl, 'jj_error 6', '(b): ' + str(errex), current_crawl_level, errorurls_man_dict)
                            if verbose_arg: print('Retry request loop(3):', workingurl)
                            continue


                # End of retry attempts
                else:
                    if verbose_arg: print('Loop exhausted:', workingurl)

                # Skip to next URL if loop is exhausted or on fatal error
                if loop_success == False:
                    print('Loop failed:', workingurl)

                    # If portal html request failed, use domain as fallback
                ...
                if red_url != workingurl:
                    if verbose_arg: print('Redirect from/to:', workingurl, red_url)
                    ...
                    if not workingurl_dup == red_url_dup:
                ...
                # Search for dynamic content urls
                if re.search("(?<!pre)fetch|xmlhttprequest", html.lower()):
                    dyn_urls_man_l.append(workingurl)
                    #if verbose_arg: print('xhr/fetch:', html.lower(), '\n:xhr/fetch', workingurl)
                ...
                for x in r:
                    #if verbose_arg: print('Decomposed:', workingurl, x)
                    x.decompose()
                ...
                vis_soup = soup.text.lower()

                if vis_soup is None:
                    if verbose_arg: print('Empty soup:', workingurl)
                    continue
                ...
                if current_crawl_level != -1:
                    ...
                    # Search for keyword on page
                    if any(zzzz in vis_soup for zzzz in keyword_list):
                        if verbose_arg: print('~~~~~~ Keyword match:', workingurl)
                        ...
                        # Append URL, jobword confidence, and portal URL to result list
                        with lock:
                            sel_q.put([workingurl, jbw_count, working_list[2]])

                    else:
                        if verbose_arg: print('~~~~~~ No keyword:', workingurl)
                ...
                for i in soup.find_all(class_='pagination'):
                    ...
                    for ii in i.find_all('a'):
                        ...
                        if ii.text.lower() == 'next':
                            ...
                            # Add to queue
                            if verbose_arg: print(workingurl, 'Adding pagination url:', abspath)
                            add_bool = True
                            proceed_f(abspath, working_list, checkedurls_man_list, verbose_arg, skipped_pages, blacklist, current_crawl_level, all_urls_q, total_count, add_bool)
                ...
                if verbose_arg: print('Starting crawler:', working_list)
                ...
                for tag in alltags:
                    ...
                    if any(www in bs_contents for www in jobwords_high_conf):
                        #if verbose_arg: print('High conf found:', workingurl, bs_contents)
                ...
                for tag in alltags:
                    ...
                    if not all_links_arg:
                        ...
                        if not any(xxx in bs_contents for xxx in jobwords_ephemeral):
                            #if verbose_arg: print('No job words detected:', workingurl, bs_contents[:99])
                            continue
                        #else: if verbose_arg: print('jbw match:', workingurl, bs_contents)

                        # Exclude if the tag contains a bunkword
                        if any(yyy in lower_tag for yyy in bunkwords):
                            if verbose_arg: print('Bunk word detected:', workingurl, lower_tag[:99])
                            continue
            ...
            # Catch all other errors
                ...
                print(os.getpid(), '---- Unknown error detected. Skipping...', str(traceback.format_exc()), workingurl)
                add_errorurls_f(workingurl, 'jj_error 1', str(errex), current_crawl_level, errorurls_man_dict)
...
['https://www3.sunysuffolk.edu/About/Employment.asp', 0, (40.8664874, -73.0356625)]
    ...
    while True:
        ...
        # Librarian shortcut
        if keyword_resp == 'l':
            keyword_list.append('librarian')
            keyword_list.append('library clerk')
            keyword_list.append('library aide')
            keyword_list.append('library media specialist')
            break
    ...
    sel_q = Queue() # URLs that need to be confirmed by Selenium

    # Create manager lists to be shared between processes
    manager = Manager()
    keywordurl_man_list = manager.list() # URLs that have been confirmed by Selenium
    sel_rejected_man_list = manager.list() # URLs that have been rejected by Selenium
    ...
    dyn_urls_man_l = manager.list() # Used to find pages with dynamic content
    ...
    sel_count = Value('i', 0) # Number of instances of Selenium


    # Create child processes
    for arb_var in range(num_procs):
        worker = Process(target=scraper, args=(keyword_list, all_urls_q, max_crawl_depth, keywordurl_man_list, checkedurls_man_list, errorurls_man_dict, skipped_pages, prog_count, total_count, all_links_arg, verbose_arg, sel_q, sel_count, jbw_tally_man_l, dyn_urls_man_l))
        worker.start()
    ...
    for i in jobwords_civ_low:
        ...
        if verbose_arg: print(i, '=', r_count)
    ...
    for i in dyn_urls_man_l:
        print(i)
    try:
        print(len(checkedurls_man_list) / len(dyn_urls_man_l), 'per cent')
    except:
        print('errrrrrr')
    ...
    if write_arg:
        ...
        jorb_home = os.path.join(expanduser("~"), 'joes_jorbs')
        if not os.path.exists(jorb_home):
            os.makedirs(jorb_home)

        # Clear errorlog
        error_path = os.path.join(jorb_home, 'jj_errorlog.txt')
        with open(error_path, "w") as error_file:
            error_file.write('')

        # Clear results
        results_path = os.path.join(jorb_home, 'jj_results.txt')
    ...
    if len(errorurls_man_dict.values()) > 0:
        ...
        print('  -----------------------------|-------------')
        print('      Error 1:   Unknown error |', error1_tally)
        ...
        print('      Error 7:  Selenium error |', error7_tally)
    ...
    if len(domain_excess_list) > 0:
        ...
        print('\n Open all', len(domain_excess_list), 'URLs in browser?\ny/n\n')
        browserresp = input()
        if browserresp.lower() == 'y' or browserresp.lower() == 'yes':
            for eachbrowserresult in domain_excess_list:
                webbrowser.open(eachbrowserresult, new=2)
                time.sleep(.4)

    # Display Selenium rejected urls
    if len(sel_rejected_man_list) > 0:
        print('\n\nSelenium rejected list:')
        for i in sel_rejected_man_list:
            print(i)
        print('\n', len(sel_rejected_man_list), 'Selenium rejected list.\n These matches have been seperated because they are probably false postives.')
        print('\n Open all', len(sel_rejected_man_list), 'URLs in browser?\ny/n\n')
        browserresp = input()
        if browserresp.lower() == 'y' or browserresp.lower() == 'yes':
            for eachbrowserresult in sel_rejected_man_list:
                webbrowser.open(eachbrowserresult, new=2)
            time.sleep(.4)
    ...
    if len(portal_error_list) > 0:
        ...
        browserresp = input()
        if browserresp.lower() == 'y' or browserresp.lower() == 'yes':
            for eachbrowserresult in portal_error_list:
                webbrowser.open(eachbrowserresult, new=2)
            time.sleep(.4)
